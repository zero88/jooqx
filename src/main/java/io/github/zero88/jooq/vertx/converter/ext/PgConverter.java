...
/**
 * In order to convert custom JSON objects into user object using the PgClient, we need a three-way-conversion. The jooq
 * converter requires us to convert between e.g. <code>org.jooq.JSONB</code> and the user-type. To obtain the data from
 * the PgClient we need a direct conversion between the PgClient-type, e.g. <code>io.vertx.core.json.JsonObject</code>
 * and the user type.
 *
