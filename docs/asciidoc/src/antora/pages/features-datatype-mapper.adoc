...
* Both same data type.
Hurray!!!
No need do anything Vertx SQL data type (a.k.a. database type) is different to `jOOQ` JDBC type, then you might want one of these solutions:
...
So, to resolve these situations, `jooqx` introduce link:core/src/main/java/io/zero88/jooqx/datatype/DataTypeMapper .java[DataTypeMapper] to make three-way-conversion that based on `jOOQ converter`.
Besides that, `jooqx` provides some default mapper utilities for make converter is easier with `UserTypeAsJooqType` and `UserTypeAsVertxType`.
...
include::partial$features-datatype-mapper.adoc[]
